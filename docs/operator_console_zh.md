...
### 4.1 紧急停止 `/api/nav/estop`

手机 UI 底部**常驻一个红色 Emergency Stop 按钮**。点一下会同步执行：

1. **立即** SIGKILL 当前的 `navigate_to_waypoint.py` 子进程（进程组级别，无宽限）。
2. 运行白名单里的 `estop_command`（默认 `ros2 run go2w_real emergency_stop.py`），
   该 helper 会：
   - 调 `/navigate_to_pose/_action/cancel_goal` 取消**所有**活跃 Nav2 目标
     （不依赖某个 goal handle，所以 `navigation_executor.py` 发的目标也会被取消）；
   - 以 20 Hz 向 `/cmd_vel` 发 `Twist(0)` 约 1.5 秒，确保底盘减速到零。
3. Helper 默认 6 秒超时；超时会在 UI 上标红失败，但子进程早已被杀。
...
```json
{
  ...
  "stdout": "[emergency_stop] requested cancel on /navigate_to_pose; ...\n"
}
```

`/api/nav/status` 里的 `last_estop` 字段会保留最近一次 E-STOP 的时间/结果。

> **注意**：E-STOP 不会关导航栈，底盘驱动、SLAM、Nav2 都还在跑。需要彻底关停
> 再 `POST /api/profiles/{name}/stop` 或在 UI 点 Stop。做这套选择是因为现场更常见
> 的需求是「让它先别动，我调整一下再继续」，而不是「从头来一次」。
...
- `emergency_stop.py` 依赖 ROS 2 的 action 内部服务路径 `<action>/_action/cancel_goal`，
  ...
  service 更短也更能一眼看懂。
- E-STOP 只解决「马上别动」。它不关栈。如果以后需要「一键掀桌」（停栈 + 停 bridge），
  加一个 `POST /api/system/kill_all` 就好，别把语义塞进 `/api/nav/estop`——两种意图
  在现场是分开的（E-STOP 要秒级响应，kill_all 本来就要几秒）。
