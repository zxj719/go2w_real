...
- 控制台里可以：
  ...
  3. 启动完整 `start_navigation_headless.sh` 链路，或启动带 RViz2 的 headless profile
  4. 从网页运行 `navigate_to_waypoint_with_debug_bag.sh`，填写目标点和 readiness/near-goal 等调参项并自动录 bag
  5. 看 Foxglove Web（`ws://<robot-ip>:8765`）做可视化；如果机器人侧部署了 noVNC/x11vnc，也可以从网页打开 RViz 远程桌面入口
...
- POI 派发走 `profiles.yaml` 顶层的 `navigation.navigate_command` 模板，字面量
  ...
  等）都来自环境变量或模板内固定写死。
- 手机端的 map profile 会通过 `debug_navigation.waypoint_files` 选择 POI 文件。当前
  `zt_0` 和 `14_1` 都使用 `/home/unitree/ros_ws/src/go2w_real/config/go2w_waypoints.yaml`，
  因此切换 profile 后 POI 列表和实际派发目标都来自这同一份文件。
...
| `navigation_headless_rviz` | `scripts/start_navigation_headless.sh --profile zt_0` + `GO2W_HEADLESS_USE_RVIZ=1` | 完整 headless 链路，同时让 `slam_rf2o.launch.py` 起 RViz2 | 不建议同时点本地 POI；更适合现场观察 |
...
| POST | `/api/nav/estop` | **紧急停止**（见 §4.1）|
| GET  | `/api/config` | 前端配置：Foxglove/RViz 链接模板、debug bag 默认参数 |
| GET  | `/api/debug/status` | `navigate_to_waypoint_with_debug_bag.sh` 当前状态 |
| GET  | `/api/debug/logs?tail=N` | debug bag wrapper 最近 N 行输出 |
| POST | `/api/debug/start` | 启动一次带录包的 debug waypoint 导航 |
| POST | `/api/debug/cancel` | 停止当前 debug bag 导航 |

`/api/debug/start` 请求示例：
...
```json
{"profile": "14_1", "waypoint_sequence": "POI_019,POI_020"}
```

或：

```json
{"profile": "zt_0", "all_waypoints": true}
```

安全边界：debug bag 后端只接受白名单 profile 和固定调参字段，不接受任意 shell 片段。
...
终态（`succeeded/failed/canceled/estopped`）会多出 `exit_code`、`finished_at`，失败时还会有
`error` 字段（从 stdout 里抓到的错误行）。紧急停止后 state = `estopped`。

### 4.1 紧急停止 `/api/nav/estop` — 核爆式关停

> 设计目标：**无论当前什么状态**（栈在跑 / 已经死 / 只 POI 导航在跑 / 栈和远端
> executor 都在跑），一键按下后机器人必须不动。所以 E-STOP 现在做三件事**并行**：
>
> 1. SIGKILL 当前 `navigate_to_waypoint.py`（毫秒级，没有宽限期）
> 2. 白名单里的 `estop_command`（默认 `ros2 run go2w_real emergency_stop.py`）：
>    - 调 `/navigate_to_pose/_action/cancel_goal` 取消所有 Nav2 活跃目标
>      （不依赖 goal handle，`navigation_executor.py` 发的目标也一并取消）
>    - 30 Hz 向 `/cmd_vel` 发 `Twist(0)` 持续约 **3 秒**——足够覆盖下面的 profile
>      卸载窗口
> 3. 同时（另起线程）把**所有**正在 running / starting 的 profile
>    `SIGTERM→SIGKILL`（走进程组 killpg），触发 shell 脚本里的 trap，连带
>    `slam_rf2o.launch.py` / `xt16_driver` / `navigation_executor.py` 都带下去

因为 2 和 3 并发执行，Nav2 在死的过程中也一直被 `Twist(0)` 覆盖，不会出现
「goal 取消了但控制器在减速过程中还残留一帧非零速度」的窗口。

Helper 默认 6 秒超时；总等待时间上限是 `2 × 6 = 12` 秒，超过就返回（防止挂死的 profile
把整个 HTTP 调用永远卡住）。
...
```json
{
  "ok": true,
  "killed_process": true,
  "stopped_profiles": ["navigation"],
  "stop_error": null,
  "exit_code": 0,
  "elapsed_seconds": 3.12,
  "helper": "done",
  "stdout": "[emergency_stop] requested cancel on /navigate_to_pose; server reports 1 goals canceling\n[emergency_stop] published 93 zero-velocity messages to /cmd_vel over 3.00s\n[emergency_stop] done\n"
}
```

`/api/nav/status` 里的 `last_estop` 字段保留最近一次 E-STOP 的时间 / ok / detail，
UI 会把它显示在按钮下方一行小字里。

**UI 上的按钮**：底部常驻，红色脉动，双击保护（进行中再点不重复触发）；如果第一次
POST 因网络抖动失败，客户端会自动重试一次，再失败才 toast 出来并提示你走 SSH
`pkill -f navigation`。

**想用命令行复现**：

```bash
curl -X POST http://<robot-ip>:8080/api/nav/estop
```

**E-STOP 之后状态是什么**：

- 所有被杀的 profile 在 `/api/profiles` 里回到 `stopped`
- 导航状态 `state=estopped`
- 机器人不动；`/cmd_vel` 没人再发就静默；底盘驱动（`go2w_bridge.py`）如果还在跑，
  看到没新命令 + cmd_vel_timeout 就保持零
- **要重新开跑只需再点一次 Start**——launch_manager 本身一直在跑，profile 停了可以随时再起
...
如果想让手机端启动机器人桌面上的 RViz2，用现成的 profile：
...
注意：RViz2 是 Qt 桌面程序，手机浏览器不能直接嵌入 RViz。网页里的 `RViz / noVNC`
链接默认指向 `http://<robot-ip>:6080/vnc.html?...`，需要机器人上另行部署 noVNC/x11vnc
或等效远程桌面服务。没有远程桌面时，手机端优先使用 Foxglove Web。

### 8.3.1 想从手机端跑带录包的 waypoint 调试

改 `profiles.yaml` 顶层的 `debug_navigation`：
...
网页会把目标点、序列、near-goal、TF/Scan/XT16 readiness 等字段发到
`/api/debug/start`。后端只把这些字段转换为固定 CLI 参数，例如：

```bash
navigate_to_waypoint_with_debug_bag.sh \
  --profile zt_0 \
  --waypoint-file /home/unitree/ros_ws/src/go2w_real/config/go2w_waypoints.yaml \
  --scan-ready-min-hz 4.0 \
  -- \
  --waypoint POI_019 \
  --near-goal-distance 0.40
```

不要把任意 shell 命令透传给手机端。
...
| `scripts/emergency_stop.py` | E-STOP 专用。无状态一次性脚本：取消所有 Nav2 goal + 发零速到 `/cmd_vel` 约 1.5s 后退出。独立于任何栈/执行器，所以在两套 profile 下都能用。 |
...
13. 再派一次目标，跑起来**在机器人还动的时候按 Emergency Stop**：状态应在 1-2 秒内
    变成 `Estopped`，底盘立即减速到零；`last_estop.ok=true`
14. Stop 栈，观察 Stack Status 回 `stopped`，Services 全部变灰

能顺利走完这 14 步，这套端侧 console 就可以交给别人用了。
...
- `emergency_stop.py` 依赖 ROS 2 的 action 内部服务路径 `<action>/_action/cancel_goal`，
  Foxy / Humble 都成立。万一以后换到某个怪版本发现这个 service 名字变了，就在脚本
  里换成 `rclpy.action.ActionClient` 的公共 API。现阶段**不要**提前做抽象——直接用
  service 更短也更能一眼看懂。
- **E-STOP 已经会关栈**（之前有一版不关栈，现在改了——现场要的是「按下去就彻底停，
  不管当前什么状态」）。实现里把 helper + `stop_running()` 放到两条线程并行跑，
  helper 持续 3 秒的 `Twist(0)` 把 profile shell 的 trap 清理窗口覆盖住。如果以后
  要回到「只停运动不关栈」的轻量版本，保留 `cancel_only=True` 之类的参数加在 body
  里，不要新建 endpoint（避免按钮变多）。
- `emergency_stop.py` 是独立脚本，**不**依赖 operator_console 或 Nav2 在跑：即使
  栈已经死了，E-STOP 按下去它也会 `rclpy.init` 出一个临时节点，往 `/cmd_vel` 猛推 3s
  的零速度。所以按了**一定**有效果，除非 ROS 2 本身（`/opt/ros/foxy`）都坏了——这
  种情况下你该 SSH 上去直接 `pkill` 而不是等按钮。
