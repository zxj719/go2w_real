...
def test_launch_defaults_to_sport_odom_ekf_for_mapped_navigation():
    ...
    assert "RF2O is kept on /rf2o/odom for diagnostics" in launch_text
    assert "/odom stays on sport odom + IMU fusion by default" in launch_text
...
def test_headless_config_pipeline_defaults_to_ekf_fusion():
    ...
    assert "use_odom_fusion: true" in config_text
    assert "HEADLESS_USE_ODOM_FUSION=1" in helper_output
    assert (
